...
export const validateGameCreation = [
  body('name').trim().isLength({ min: 1, max: 100 }).withMessage('El nombre debe tener entre 1 y 100 caracteres'),
  body('description').optional().trim().isLength({ max: 1000 }).withMessage('La descripción no puede superar los 1000 caracteres'),
  body('externalId').optional().trim(),
  body('thumbnail').optional().trim().isURL().withMessage('El thumbnail debe ser una URL válida'),
...
export const validateNumericParam = [
  param('id').trim().matches(/^\d+$/).withMessage('El ID debe ser un valor numérico'),
  handleValidationErrors
];
